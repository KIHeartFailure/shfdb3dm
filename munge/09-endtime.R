...
hfsos <- patreg %>% filter(DIA_all != "") %>% 
  mutate(tmp_hfsos = stringr::str_detect(DIA_all, " I110| I130| I132| I255| I420| I423| 1425| I426| I427| I428| I429| I43| I50| J81| K761| 414W| 425E| 425F| 425G| 425H| 425W| 425X| 428")) %>%
  filter(tmp_hfsos)
...
rsdata <- rsdata %>%
  mutate(censdtm = coalesce(pmin(sos_deathdtm, tmp_migrationdtm, na.rm = TRUE), 
                                ymd("2018-12-31")),
         censdtm = pmin(censdtm, tmp_hfsosdtm, na.rm = TRUE),
         censdtm = pmin(censdtm, tmp_hfrsdtm, na.rm = TRUE),
         tmp_diff = tmp_hfsosdtm - shf_indexdtm
  ) %>% 
  filter(tmp_diff > 0 | is.na(tmp_diff)) %>% # finns 23 fall som har sos hf diagnos innan de blir controller. delete.
  select(-shf_deathdtm) 
